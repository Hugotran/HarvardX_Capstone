...
```{r What will happen if we have better dataset, echo = TRUE}
library(tidyverse)
library(Metrics)
library(caret)
...
test_data <- test_data %>% 
  semi_join(subset_train_data, by = "movieId") %>%
  semi_join(subset_train_data, by = "userId")
...
rmses <- sapply(lambdas, function(l){
  ...
  mean_rating <- mean(subset_train_data$rating)
  
  b_i <- subset_train_data %>% 
    group_by(movieId) %>%
    summarize(b_i = sum(rating - mean_rating)/(n()+l))
  
  b_u <- subset_train_data %>% 
